...
{
    public partial class DichVietAnh : UserControl
    {
        string[] arrNghia;
        ...
        {
            InitializeComponent();
            arrTu = System.IO.File.ReadAllLines("viet3.txt");

            string line2;
            line2 = System.IO.File.ReadAllText("vietanh.txt");

            arrNghia = line2.Split(new char[] { '@' });
            SetDataToCollection();
        }
        ...
        private void DichTu()
        {
            int i = Array.IndexOf(arrTu, txtNhapTu.Text.ToLower());
            if (i < 0)
            {
                rtbTu.Text = "\nKhông có từ này";
            }
        ...
        private void DichVanBan()
        {
            try
            {
                ...
                else
                {
                    ...
                    rtbVB.Text = "\n" + client.Translate("6CE9C85A41571C050C379F60DA173D286384E0F2", txtNhapVB.Text, "", "en");
        ...
        {
            voice.Speak(rtbVB.Text, SpeechVoiceSpeakFlags.SVSFDefault);
        }
